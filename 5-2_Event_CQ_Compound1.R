...
events <- with_tz(read_csv('DataFiles/hydro_data/event_delineations/events_compound1.csv'), 'America/Chicago') %>%
...
event_data <- all_hydro_15min %>%
  filter(!is.na(event_compound1)) %>% filter(is.na(riverQ_ingap)) %>%
  #smooth N
...
q_step <- tibble(Q_norm = rep(seq(from = 0, to = 100)/100, nrow(events)),
                 event_compound1 = rep(events$event_number, each = 101))
...
rise_limbs <- event_data %>%
  ...
  full_join(q_step)  %>%
  arrange(event_compound1, Q_norm) %>%
  select(dateTime, event_compound1, riverQ_linterp, Q_norm, N, N_smooth_norm) %>%
  ...
  select(event_compound1, dateTime, Q_norm, rise_N_norm = N_smooth_norm)
...
fall_limbs <- event_data %>%
  ...
  #select only q_steps
  filter(Q_norm %in% (seq(0:100)/100)) %>%
  select(event_compound1, dateTime, Q_norm, fall_N_norm = N_smooth_norm)
...
CQ_summary_compound1 <- hyst_summary %>%
  left_join(flush)
write_csv(CQ_summary_compound1, './DataFiles/hydro_data/CQ_summary_compound1.csv')




# Calculate Predictor Variables -------------------------------------------
CQ_summary_compound1 <- with_tz(read_csv('./DataFiles/hydro_data/CQ_summary_compound1.csv'), 'America/Chicago')

#Times/seasonality
event_temporal <- events %>%
  mutate(event_compound1 = event_number,
  ...
  select(event_compound1, start_dateTime, end_dateTime, wateryear, month, season, duration)
#join to event summary
event_summary <- CQ_summary_compound1 %>%
...
write_csv(event_summary, './DataFiles/hydro_data/event_summary_compound1.csv')


# Plotting ----------------------------------------------------------------

#Load Data
event_summary_compound1 <- with_tz(read_csv('./DataFiles/hydro_data/event_summary_compound1.csv'), 'America/Chicago') %>%
  mutate(wateryear = factor(ifelse(as.integer(month(start_dateTime)) >= 10, as.integer(year(start_dateTime)) + 1, year(start_dateTime))),
         month = factor(month(start_dateTime, label = TRUE)),
         season = factor(month(floor_date(start_dateTime, unit = 'season')), levels = c(12,3,6,9), labels = c('Winter', 'Spring', 'Summer', 'Fall')))

#Monthly averages
monthly_averages_compound1 <- event_summary_compound1 %>%
  group_by(month) %>%
...
#Seasonal averages
seasonal_averages_compound1 <- event_summary_compound1 %>%
  group_by(season) %>%
  summarise(mean_FI = mean(FI, na.rm = TRUE),
            sd_FI = sd(FI, na.rm = TRUE),
            mean_HI = mean(HI_mean, na.rm = TRUE),
            sd_HI = sd(HI_mean, na.rm = TRUE))
#Yearly averages
yearly_averages_compound1 <- event_summary_compound1 %>%
  group_by(wateryear) %>%
  summarise(mean_FI = mean(FI, na.rm = TRUE),
            sd_FI = sd(FI, na.rm = TRUE),
            mean_HI = mean(HI_mean, na.rm = TRUE),
...
#HI histogram
ggplot(data = event_summary_compound1) +
...
ggplot(data = event_summary_compound1) +
  ...
  geom_errorbar(data = monthly_averages_compound1, aes(y = mean_HI, xmin = mean_FI - sd_FI, xmax = mean_FI + sd_FI), alpha = 0.3) +
  geom_errorbar(data = monthly_averages_compound1, aes(x = mean_FI, ymin = mean_HI - sd_HI, ymax = mean_HI + sd_HI), alpha = 0.3) +
  geom_point(data = monthly_averages_compound1, aes(x = mean_FI, y = mean_HI), shape = 15, size = 2) +
...
ggplot(data = event_summary_compound1) +
  geom_point(aes(x = FI, y = HI_mean, color = season)) +
  geom_errorbar(data = seasonal_averages_compound1, aes(y = mean_HI, xmin = mean_FI - sd_FI, xmax = mean_FI + sd_FI), alpha = 0.3) +
  geom_errorbar(data = seasonal_averages_compound1, aes(x = mean_FI, ymin = mean_HI - sd_HI, ymax = mean_HI + sd_HI), alpha = 0.3) +
  geom_point(data = seasonal_averages_compound1, aes(x = mean_FI, y = mean_HI, color = season), shape = 15, size = 2) +
  scale_color_manual(values = c('#8DA9C4','#60992D','#C33C54','#F4B860'), name = "Season") +
  geom_hline(yintercept = 0) +
  geom_vline(xintercept = 0) +
  scale_x_continuous(limits = c(-1,1)) +
...
ggplot(data = event_summary_compound1) +
  ...
  geom_errorbar(data = yearly_averages_compound1, aes(y = mean_HI, xmin = mean_FI - sd_FI, xmax = mean_FI + sd_FI), alpha = 0.3) +
  geom_errorbar(data = yearly_averages_compound1, aes(x = mean_FI, ymin = mean_HI - sd_HI, ymax = mean_HI + sd_HI), alpha = 0.3) +
  geom_point(data = yearly_averages_compound1, aes(x = mean_FI, y = mean_HI, color = wateryear), shape = 15, size = 2) +
  geom_hline(yintercept = 0) +
  geom_vline(xintercept = 0) +
  scale_x_continuous(limits = c(-1,1)) +
  scale_y_continuous(limits = c(-1,1)) +
...
ggplot(data = subset(event_summary_compound1, !is.nan(HI_mean))) +
