...
event_temporal <- events %>%
  mutate(event = row_number(start_dateTime),
  ...
  select(event, start_dateTime, end_dateTime, wateryear, month, season, duration)
#join to event summary
event_summary <- CQ_summary %>%
...
ggplot(data = event_summary) +
  ...
  geom_errorbar(data = monthly_averages, aes(y = mean_HI, xmin = mean_FI - sd_FI, xmax = mean_FI + sd_FI), alpha = 0.3) +
  geom_errorbar(data = monthly_averages, aes(x = mean_FI, ymin = mean_HI - sd_HI, ymax = mean_HI + sd_HI), alpha = 0.3) +
  geom_point(data = monthly_averages, aes(x = mean_FI, y = mean_HI), shape = 15, size = 2) +
...
ggplot(data = event_summary) +
  ...
  geom_point(data = seasonal_averages, aes(x = mean_FI, y = mean_HI, color = season), shape = 15, size = 2) +
  scale_color_manual(values = c('#8DA9C4','#60992D','#C33C54','#F4B860'), name = "Season") +
  geom_hline(yintercept = 0) +
  geom_vline(xintercept = 0) +
  scale_x_continuous(limits = c(-1,1)) +
...
ggplot(data = event_summary) +
  ...
  geom_errorbar(data = yearly_averages, aes(y = mean_HI, xmin = mean_FI - sd_FI, xmax = mean_FI + sd_FI), alpha = 0.3) +
  geom_errorbar(data = yearly_averages, aes(x = mean_FI, ymin = mean_HI - sd_HI, ymax = mean_HI + sd_HI), alpha = 0.3) +
  geom_point(data = yearly_averages, aes(x = mean_FI, y = mean_HI, color = wateryear), shape = 15, size = 2) +
  geom_hline(yintercept = 0) +
  geom_vline(xintercept = 0) +
  scale_x_continuous(limits = c(-1,1)) +
  scale_y_continuous(limits = c(-1,1)) +
