...
# PCA ---------------------------------------------------------------------
pca_dat <- select(event_summary_cluster,
                  initial_Q, initial_N, duration_since_last, d_10260015_spei12, d_10270104_spei3, delta_rat_Q, duration,
                  total_Q, max_Q, p_10270104_1d_total, p_10260015_1d_total, p_max_y, p_max_x, r_t_1d_total, r_t_share,
                  r_m_1d_total, r_m_share, r_t_1d_delta_rat, reservoir_precip_ratio, reservoir_runoff_ratio)
...
pca_cors <- get_pca_var(pca)$cor
pca_loads <- get_pca_var(pca)$contrib

coords <- get_pca_ind(pca)$coord %>%
  cbind(., event_summary_cluster)

ggplot(data = coords) +
  geom_point(aes(x = Dim.1, y = Dim.2, color = FI)) +
